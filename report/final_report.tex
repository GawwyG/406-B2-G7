...
\title{\includegraphics[width=2.6cm]{Images/buet_logo.png}\\[0.6em]
...
In the emulated topology (Section~\ref{sec:impl}), we reproduce ARP spoofing's
...
Section~\ref{sec:observed}). Only the case -- a forged RST
delivered to the \emph{client}, spoofed as the server -- is implemented
and evaluated; the converse (spoofing the client toward the server) is not required and not evaluated here.
...
been transferred. The TCP connection itself dies immediately when the
forged RST lands, but the player's \emph{visible} freeze is not
instantaneous: like any progressive-download client, it reads ahead into
a local buffer faster than it plays the video back, so it keeps
displaying already-buffered frames at normal speed for a few seconds
after the kill, and only visibly freezes or errors out once that buffer
is exhausted and no further data can arrive. Any subsequent playback
attempt over the same (now-dead) connection should fail immediately.
Because the design depends on winning a race
...
\begin{subfigure}{0.48\textwidth}
  ...
  \caption{Client: playback frozen once its read-ahead buffer runs out}
\end{subfigure}
\caption{The attack: a single forged RST (left, note
\texttt{attempts=1}) kills the TCP connection immediately, but the
client's playback freezes only afterward (right) -- it keeps playing
normally from data already buffered before the kill, and only visibly
freezes once that buffer is exhausted.}
\label{fig:during}
\end{figure}
...
\end{lstlisting}
A single forged RST killed the connection. The attacker also observed
the client itself send a RST back to the server shortly after
(\texttt{closed itself (flags=R)} above) -- confirmation of the kill
visible without looking at the victim's screen, and consistent with
...
\end{lstlisting}
Only 678{,}920 of the expected 3{,}985{,}444 bytes ($\approx$17\%) were
received -- the player's own confirmation of an abnormal severance, not
just a slow download (which would not produce a ``partial file'' error).
With \texttt{curl} instead of a GUI player, the equivalent signal is a
non-zero exit code, e.g.\ \texttt{56}, ``Recv failure: Connection reset
by peer''.
...
\end{lstlisting}
The server keeps sending into a connection already dead on the
client's end, until its own timeout logic notices. This asymmetry --
the attacker targets the client, not the server -- follows directly
from the attack's design (Section~\ref{sec:forge}).

\subsection{Quantitative Results}
\label{sec:quant}

An automated harness drives the whole topology through $N$ repeated
trials per condition, avoiding reliance on a single anecdotal demo:
\begin{itemize}[nosep]
  \item \textbf{Success is measured objectively} -- downloaded byte
    count vs.\ known full size, plus the client's exit code -- not by
    eyeballing playback.
  \item \textbf{Each trial gives the download a head start} proportional
    to the estimated total transfer time before starting the attacker,
    modeling on-path visibility gained partway through an already-flowing
    stream (as ARP spoofing would produce), not from before the handshake.
\end{itemize}

Table~\ref{tab:sweep}: success rate and mean attempts-to-kill across WAN
bandwidths (20 trials/condition, defense disabled).
...
Packet captures on the victim confirm \emph{why} it wins: the forged
RST's sequence number lands exactly on the real stream's current byte
boundary every time (never a guess), arriving 5.5\,ms, 10\,ms, and
3.2\,ms ahead of the real next segment across three captured examples --
direct, measured evidence of the LAN-vs-WAN path advantage
(Section~\ref{sec:impl}).

\subsection{Expected vs.\ Observed Outcome, and Where They Differ}
\label{sec:diverge}

For 1--50\,Mbit/s (poor mobile to high-quality streaming), observed
matched expected exactly: deterministic, one-shot success every trial.
Two divergences are worth reporting, since understanding why is itself
informative.

\paragraph{Divergence 1: implementation latency, not network
conditions, was the first bottleneck.}
\label{sec:latency}
An initial Scapy-based attacker showed a bimodal pattern under repeated
trials -- some connections died in one shot, others fired 600+ times
and never landed a hit. A synchronized victim-side capture found why:
Scapy's per-packet overhead was close to or above the real
inter-segment gap ($\approx$24\,ms at 1\,Mbit/s), so once one shot was
slow, a processing backlog formed and never shrank -- a textbook
unstable queue. Reaction latency climbed from 27\,ms to over 16
\emph{seconds} across one losing trial.

\textbf{Fix:} raw \texttt{AF\_PACKET} sockets (no per-packet Scapy
overhead) plus always processing the newest queued packet, not the
oldest (bounds the worst case to one stale cycle, not an ever-growing
backlog). Reaction latency dropped to 0.1--10\,ms, and behavior became
the near-deterministic one-shot success in Table~\ref{tab:sweep}. The
divergence was an implementation artifact, not a design flaw --
removing it made observed behavior match the prediction.

\paragraph{Divergence 2: the race flips at unrealistically high
bandwidth.} At 100\,Mbit/s, success dropped to 0\% (0/11) -- the
opposite of every lower bandwidth. Same race-margin model explains it:
the segment gap shrinks to $\approx$0.23\,ms, now comparable to the
attacker's own processing latency, so the topological advantage alone
is no longer enough. Not a design flaw -- success was always framed as
winning a race, not a guaranteed exploit -- just a measured boundary,
and one well outside realistic streaming bitrates.
...
project tool; only the forged RST delivered to the client was
implemented and evaluated, not the converse (spoofing the client toward
the server instead); and the attacker deliberately never suppresses or
delays legitimate traffic, only observes and injects.
...
extensions include the converse attack (spoofing the client toward the
server), an off-path sequence-guessing variant (to concretely exercise
the challenge-ACK path RFC 5961 provides), and TTL-based anomaly
detection as an additional, complementary defense layer.
...
Member & Contribution \\
\midrule
2105094: Omar Abdur Razzaque &
\begin{minipage}[t]{9.5cm}
\begin{itemize}[leftmargin=*, nosep, topsep=0.3em]
  \item Project scaffolding and repository setup
  \item Mininet LAN/WAN topology
  \item Flask streaming server and test video
  \item Playback verification and baseline packet capture
  \item Attacker visibility (OVS mirror + sniffer)
  \item Attack implementation
  \item Final Report
\end{itemize}
\end{minipage} \\[0.8em]
\cmidrule{1-2}
\\[0.8em]
2105104: Rubaiyat-E-Zaman &
\begin{minipage}[t]{10.5cm}
  \begin{itemize}[leftmargin=*, nosep, topsep=0.3em]
  \item Attack implementation
  \item Reliability fix (raw-socket rewrite, drain-to-latest)
  \item Hand-rolled ARP resolution (removed last library dependency)
  \item Switch-side IP Source Guard defense
  \item Automated experiments and quantitative trials
  \item Final report
\end{itemize}
\end{minipage} \\[0.3em]
\bottomrule
